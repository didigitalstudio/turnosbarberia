...
export default async function HomePage() {
  ...
  const greetingName = profile?.name?.split(' ')[0] || 'Hola';
  ...
  return (
    <main className="min-h-screen flex flex-col">
      <header className="px-5 pt-3 pb-3 flex items-center justify-between">
        ...
        <div className="relative grid place-items-center w-10 h-10 rounded-full bg-card border border-line">
      ...
      <div className="flex-1 overflow-auto px-5 pb-6">
        ...
        {next ? (
          <div className="bg-ink text-bg rounded-3xl px-5 py-5 mt-1 relative overflow-hidden">
            ...
            <div className="flex items-baseline gap-2.5 mt-2.5">
              ...
              <div className="font-display text-[30px] italic" style={{ color: '#B6754C' }}>
            ...
            <div className="flex gap-2 mt-4.5">
              <Link href="/mis-turnos" className="flex-1 bg-transparent text-bg border border-dark-line px-3 py-2.5 rounded-m text-[13px] font-medium text-center">Reprogramar</Link>
              <Link href="/mis-turnos" className="flex-1 bg-bg text-ink px-3 py-2.5 rounded-m text-[13px] font-semibold text-center">Ver detalles</Link>
            </div>
          </div>
        ) : (
          <div className="bg-ink text-bg rounded-3xl px-5 py-6 mt-1 relative overflow-hidden">
            ...
            <div className="font-display text-[28px] leading-tight mt-2">Reservá tu próximo corte</div>
            <div className="text-[12px] text-dark-muted mt-2 max-w-[260px]">Te lleva 30 segundos.</div>
          </div>
        )}

        <Link href="/reservar"
          className="block w-full mt-3.5 bg-accent text-white border-0 px-4 py-4 rounded-2xl text-[15px] font-semibold flex items-center justify-center gap-2.5">
          <Icon name="plus" size={18}/> Reservar nuevo turno
        </Link>

        {/* Services */}
        <div className="mt-6 flex items-baseline justify-between">
          <div className="font-display text-[22px] -tracking-[0.3px]">Servicios</div>
          <Link href="/reservar" className="text-xs text-muted">Ver todos</Link>
        </div>
        <div className="flex flex-col gap-2 mt-3">
          {(services || []).slice(0,4).map(s => (
            <Link key={s.id} href={`/reservar?service=${s.id}`}
        ...
        <div className="mt-5 flex items-baseline justify-between">
          <div className="font-display text-[22px] -tracking-[0.3px]">Nuestro equipo</div>
        </div>
        <div className="flex gap-2.5 mt-3 overflow-x-auto no-scrollbar">
          {(barbers || []).map(b => (
            <div key={b.id} className="min-w-[132px] bg-card border border-line rounded-xl p-3.5 text-center">
